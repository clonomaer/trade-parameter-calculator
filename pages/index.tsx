...
const Home: NextPage = () => {
    ...
    //         id: subType.id,
    //         fields: [] as { id: string; subject: BehaviorSubject<string> }[],
    //     })),
    // )

    // useEffect(() => {
    //     fieldSubjects.current.forEach(subType => {
    //         subType.fields.push(
    //             ...subType.fields.map(field => ({
    //                 id: field.id,
    //                 subject: new BehaviorSubject<string>(
    //                     (
    //                         JSON.parse(localStorage.getItem('fields')) as
    //                             | null
    //                             | {
    ...
    //                                   }[]
    //                               }
    //                     )?.[subType.id].find(x => x.id === field.id)?.value ??
    //                         '',
    //                 ),
    //             })),
    //         )
    ...
    // useEffect(() => {
    //     consol0e.log('local storage:', localStorage.getItem('fields'))
    //     const fields = JSON.parse(localStorage.getItem('fields')) as
    //         | null
    //         | { [x in PositionSubType]: { id: string; value: string }[] }
    //     if (!_.isNil(fields)) {
    //         console.log('nexting shit', fields)
    //         fieldSubjects.current.forEach(
    //             ({ id: subTypeId, fields: subTypeFields }) => {
    //                 subTypeFields.forEach(({ id, subject }) => {
    //                     const value = fields[subTypeId]?.find(
    //                         ({ id: _id }) => id === _id,
    //                     )?.value
    //                     if (!_.isNil(value)) {
    //                         subject.next(value)
    ...
    // const getAllFieldsSubject = useMemo(
    //     () => () =>
    //         combineLatest(
    ...
    //                     }),
    //                     {} as { [x: string]: Observable<string> },
    //                 ),
    //         ).pipe(shareReplay(1)),
    //     [],
    // )
    // const allFieldsSubject = useLazyRef(getAllFieldsSubject)
    // useEffect(() => {
    //     allFieldsSubject.current = getAllFieldsSubject()
    // }, [positionSubType])

    // useEffect(() => {
    //     let sub: Subscription
    //     setTimeout(() => {
    //         sub = allFieldsSubject.current.subscribe(val => {
    //             localStorage.setItem('fields', JSON.stringify(val))
    //         })
    //     }, 5000)
